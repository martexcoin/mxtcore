...
var mxtcoreDefaults = require('../config');
...
function PeerManager(config) {
  ...
  this.config = config || {};
  for (var i in mxtcoreDefaults)
    if (mxtcoreDefaults.hasOwnProperty(i) && this.config[i] === undefined)
      this.config[i] = mxtcoreDefaults[i];
